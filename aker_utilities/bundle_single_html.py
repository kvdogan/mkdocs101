...
def log(s, color=None, on_color=None, attrs=None, new_line=True):
    if not color:
        ...
        print(str(s), color, on_color, attrs, end=" ", file=sys.stderr)
    if new_line:
        sys.stderr.write("\n")
    sys.stderr.flush()
...
def get(
    ...
    if index.startswith("http") or (relpath and relpath.startswith("http")):
        ...
        if username and password:
            auth = requests.auth.HTTPBasicAuth(username, password)  # type: ignore
        ...
        if relpath:
            ...
            if os.path.exists(relpath):
                ...
                full_path = os.path.normpath(
                    os.path.join(os.path.dirname(index), relpath)
                )
            try:
                ...
                if verbose:
                    log(
                        "[ WARN ] file not found - %s %s" % (full_path, str(err)),
                        "yellow",
                    )
                return "", None
        else:
...
def data_to_base64(index, src, verbose=True):
    ...
    if data:
        if type(data) is bytes:
            ...
            return ("data:%s;base64," % fmt) + bytes.decode(
                base64.b64encode(str.encode(data))
            )
    else:
...
def handle_css_content(index, css, verbose=True):
    ...
    if not isinstance(css, str):
        ...
        if mo:
            try:
                css = css.decode(mo.group(1))  # type: ignore
            except AttributeError:
                log(
                    "[ WARN ] failed to convert css to encoding %s" % mo.group(1),
                    "yellow",
                )
            finally:
                print("Program continues")

    # Watch out! how to handle urls which contain parentheses inside?
    # Oh god, css does not support such kind of urls
    ...
    def repl(matchobj):
        ...
        # if src.lower().endswith('woff') or src.lower().endswith('ttf') or src.lower(
        # ).endswith('otf') or src.lower().endswith('eot'):
...
def generate(
    ...
    **kwargs
    ...
    # Usage in code
    html = generate(
        r"C:/Users/kahdog/OneDrive - Aker BP/Desktop/SAPFull/index.html",
        comment=False,
        keep_script=True,
        prettify=True
    )
    ...
    with open(
        "C:/Users/kahdog/OneDrive - Aker BP/Desktop/SAPFull/indexx.html", "w",
        encoding="utf-8"
    ) as ff:
        ff.write(html)
    """
    html_doc, extra_data = get(
        index,
        verbose=verbose,
        verify=verify,
    ...
    for link in soup("link"):
        if link.get("href"):
            if (
                ...
                new_css_content = handle_css_content(
                    absurl(index, link["href"]), css_data, verbose=verbose
                )
                ...
                # if new_css_content.find(
                #     '@font-face') > -1 or new_css_content.find('@FONT-FACE') > -1:
                if False:
                    link["href"] = "data:text/css;base64," + base64.b64encode(
                        new_css_content
                    )
                else:
    ...
    for js in soup("script"):
        ...
        if not js.get("src"):
            continue
        new_type = (
            "text/javascript" if not js.has_attr("type") or not js["type"] else js["type"]
        )
    ...
    for img in soup("img"):
        ...
        # attribute, Maybe add a flag to enable the base64 conversion of each `srcset`?
        ...
        if img.get("srcset"):
            ...
            if verbose:
                log(
                    "[ WARN ] srcset found in img tag. Attribute will be cleared in => %s"
                    % (img["data-src"]),
                    "yellow",
                )
        ...
        def check_alt(attr):
            if img.has_attr(attr) and img[attr].startswith("this.src="):
                ...
                if verbose:
                    log(
                        "[ WARN ] %s found in img tag and unhandled, which may break page"
                        % (attr),
                        "yellow",
                    )
    ...
    if comment:
        for html in soup("html"):
            html.insert(
                0,
                BeautifulSoup(
                    "<!-- \n single html processed by kvdogan\n "
                    "title: %s\n url: %s\n date: %s\n-->"
...
Usage:
    ...
    $ bundle_single_html [options] some_url
...
Options:
    ...
    -c, --comments          Insert some info into comments
    -o, --output            Output filepath
    -u, --username          Use HTTP basic auth with specified username
    -p, --password          Use HTTP basic auth with specified password
    -e, --errorpage         Crawl an error page
"""
    )
...
examples: str = """
examples:
    $ bundle_single_html -h
        you are reading this help message

    $ bundle_single_html index.html -s -c -o ../indexx.html
        you are reading this help message

    $ bundle_single_html http://www.google.com > google.html
        save google index page for offline reading, keep style untainted

    $ bundle_single_html -s http://gabrielecirulli.github.io/2048/ > 2048.html
        save dynamic page with Javascript example
        the 2048 game can be played offline after being saved

    $ bundle_single_html /path/to/xxx.html > xxx_single.html
        combine local saved xxx.html with a directory named xxx_files together
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="Bundle webapp as single html",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        # usage=example_printer(),
        epilog=examples,
        add_help=True,
    )
    ...
    parser.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Don't show verbose url get log in stderr",
    )
    parser.add_argument(
        "-s",
        "--script",
        action="store_true",
        help="Keep javascript in the generated html",
    )
    parser.add_argument(
        "-k", "--insecure", action="store_true", help="Ignore the certificate"
    )
    parser.add_argument(
        "-c", "--comments", action="store_true", help="Insert some info into html head"
    )

    parser.add_argument(
        "-e", "--errorpage", action="store_true", help="crawl an error page"
    )

    parser.add_argument("-o", "--output", help="Save output to")
    parser.add_argument(
        "-u", "--username", help="Use HTTP basic auth with specified username"
    )
    parser.add_argument(
        "-p", "--password", help="use HTTP basic auth with specified password"
    )
